...
// Add routes (endpoints) and middleware functions to handle requests and perform tasks
// like authentication or logging.
...
app.get("/", (req, res) => {
  res.render("index.ejs", { recipe: dataUsed });
});
...
app.post("/recipe", (req, res) => {
  ...
  if (req.body.choice == "chicken") {
    dataUsed = JSON.parse(recipeJSON)[0];
  } else if (req.body.choice == "beef") {
    dataUsed = JSON.parse(recipeJSON)[1];
  } else if (req.body.choice == "fish") {
    dataUsed = JSON.parse(recipeJSON)[2];
  }
